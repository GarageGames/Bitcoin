...
namespace MinerLib_cs
{
    public class Scrypt
    {
        ...
        public byte[] GetHash()
        {
            ...
            for (int i = 0; i < 8; i++)
                swapped[i] = ByteReverse(output[i]);
            byte[] outHash = new byte[32];
            Buffer.BlockCopy(swapped, 0, outHash, 0, 32);
        ...
        static void sha256_block(uint[] output, uint[] state, uint[] input, uint inputOffset = 0)
        {
        ...
        static uint ROTATE(uint a, int n)
        {
